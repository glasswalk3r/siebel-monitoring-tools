...

{
    ...
    *sym = $Siebel::Srvrmgr::IPC::{EXPORT_OK};
    is_deeply(
        \@sym,
        \@expected, 
        'module exports only expected functions'
    );
}
...
note("STDOUT: $stdout");
note("STDERR: $stderr");
( $message, $is_error ) = Siebel::Srvrmgr::IPC::check_system(${^CHILD_ERROR_NATIVE});
ok(defined($message), 'message is defined');
is($message, 'Child process terminate with call to exit() with return code = 0', '"benign" error message after executing hostname');
...
( $message, $is_error ) = Siebel::Srvrmgr::IPC::check_system(${^CHILD_ERROR_NATIVE});
ok(defined($message), 'message is defined');
is($message, 'Child process terminate with call to exit() with return code = 2', '"benign" error message after executing hostname');
ok(defined($is_error), 'confirmation of ok/error is defined');
